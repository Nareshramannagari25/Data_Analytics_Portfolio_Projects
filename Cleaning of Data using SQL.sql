...
---Alternative way to Remove the Duplicates :
  ...
  --If there is an unique identifier column[ID] for each row the we can use this query to remove duplicates
	DELETE FROM dbo.[sales 2017-2019] 
	WHERE ID NOT IN 
	                  (SELECT MIN(ID)
	                    FROM(
	                         SELECT order_id, product_id, store_id, order_date, sales, revenue, stock, price,delivery_date
                             FROM dbo.[sales 2017-2019]
                             GROUP BY order_id,product_id,store_id,order_date,sales,revenue,stock,price,delivery_date
                             ORDER BY order_id ASC ) AS Inner_query);
